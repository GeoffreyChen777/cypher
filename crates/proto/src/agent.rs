...
pub struct RunRequest {
    ...
    /// Absolute paths of attachments (images or any other file) already
    /// staged on the run device (composer uploads: UploadChunk/UploadCommit →
    /// durable path). The same paths also ride the prompt text as the
    /// [`crate::attachment_refs`] trailer (zeron's `withAttachments` transport
    /// — that's what persists in the doc); this field additionally lets a
    /// harness inline image bytes as image content blocks. Additive +
    /// serde-defaulted for wire compat.
...
/// blocking `CreateWorktree` RPC — so the send path stays durable: a lost
/// relay frame can't wedge the composer on "Sending…" while the session runs
/// anyway. A request for a new worktree must either produce that worktree or
/// the command is Rejected — it never silently runs in the base checkout.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorktreeSpec {
    /// The repo whose worktree to create (the space's folder on the host).
    pub repo_path: String,
    /// Base ref the fresh `cypher/<name>` branch is created off.
    pub base_ref: String,
    /// Optional human-readable name hint for the worktree folder; the stable
    /// chat key still determines reuse (a retry never mints a second
    /// worktree).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    fn run_request_worktree_default_and_round_trip() {
        ...
        let req = RunRequest {
            worktree: Some(WorktreeSpec {
                ...
                base_ref: "main".into(),
                name_hint: Some("my-feature".into()),
            }),
            ..req
        };
        let value = serde_json::to_value(&req).unwrap();
        ...
        // Missing optionals (nameHint) parse fine.
        let bare = serde_json::json!({
            "prompt": "p",
            "cwd": ".",
            "sandbox": "workspace-write",
            "worktree": { "repoPath": "/r", "baseRef": "main" },
        });
        let parsed: RunRequest = serde_json::from_value(bare).unwrap();
        assert_eq!(
            parsed.worktree,
            Some(WorktreeSpec {
                repo_path: "/r".into(),
