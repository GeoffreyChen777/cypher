...
use crate::{AgentEvent, ToolCall, ToolDiff, UserInputQuestion};

/// Bounded message segment budget. Shared by both document import and the
/// storage-independent transcript writer; not a transport frame-size limit.
...
/// Render-only transcript projection. Never publish raw write/edit/MCP inputs;
/// keep the existing output summaries, progress tails, diffs and sidecar refs.
/// Available to typed sync and the current engine without a document dependency.
pub fn render_parts(parts: &[MessagePart]) -> Vec<MessagePart> {
    parts
        .iter()
        .map(|part| match part {
            MessagePart::Tool {
