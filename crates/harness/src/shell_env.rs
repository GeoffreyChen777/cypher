...
//!
//! Set `CYPHER_NO_LOGIN_SHELL=1` to disable the snapshot entirely.
...
mod unix {
    ...
    const BEGIN_MARKER: &str = "__CYPHER_SHELL_ENV_BEGIN__";
    const END_MARKER: &str = "__CYPHER_SHELL_ENV_END__";
    ...
    pub(super) fn capture() -> Option<OsString> {
        if cypher_env::var_os("NO_LOGIN_SHELL").is_some_and(|v| !v.is_empty()) {
            return None;
        }
    ...
    fn run_and_capture(shell: &Path, flags: &[&str], script: &str, timeout: Duration) -> Vec<u8> {
        ...
        cmd.args(flags)
            ...
            .env("CYPHER_RESOLVING_ENVIRONMENT", "1")
            .env("TERM", "dumb");
        ...
        if let Some(mut stdout) = child.stdout.take() {
            ...
            let _ = std::thread::Builder::new()
                .name("cypher-shell-env-read".into())
                .spawn(move || {
...
"#;
        ...
        fn snapshots_path_from_fake_shell() {
            ...
            let shell = fake_shell(
                dir.path(),
                &format!(
                    "#!/bin/sh\nPATH=\"/cypher-test/custom/bin:/usr/bin:/bin\"; export PATH\n{RUN_PAYLOAD}"
                ),
            );
            let path = snapshot_path(&shell, Duration::from_secs(10)).unwrap();
            let path = path.to_string_lossy();
            assert!(path.starts_with("/cypher-test/custom/bin:"), "got: {path}");
        }
        ...
        fn falls_back_when_interactive_attempt_hangs() {
            ...
            let shell = fake_shell(
                dir.path(),
                &format!(
                    "#!/bin/sh\ncase \" $* \" in *\" -i \"*) sleep 60;; esac\nPATH=\"/cypher-test/fallback/bin:/usr/bin:/bin\"; export PATH\n{RUN_PAYLOAD}"
                ),
            );
            ...
            assert!(
                path.to_string_lossy()
                    .starts_with("/cypher-test/fallback/bin"),
                "got: {}",
                path.to_string_lossy()
            );
