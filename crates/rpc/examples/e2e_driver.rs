...
fn apply_transcript_item(
    current: &mut Vec<cypher_doc::SessionMessageEntry>,
    item: &serde_json::Value,
) -> Result<(), String> {
    let frame: cypher_doc::TranscriptFrame =
        serde_json::from_value(item.clone()).map_err(|err| err.to_string())?;
    cypher_doc::apply_transcript_frame(current, frame).map_err(|err| err.to_string())
}
    ...
    let (by_device, text) = wait_stream(
        ...
        |item| {
            ...
            let entry = transcript.iter().find(|entry| {
                entry.role == cypher_doc::MessageRole::Assistant
                    && entry.status == Some(cypher_doc::MessageStatus::Complete)
            })?;
...
mod tests {
    ...
    fn transcript_reset_frame_is_materialized() {
        ...
        assert_eq!(
            transcript[0].status,
            Some(cypher_doc::MessageStatus::Complete)
        );
    }
}
