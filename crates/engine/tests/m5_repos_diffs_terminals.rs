...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: cypher/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("cypher/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's cypher/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, cypher branch removed, refs pruned.
    repos
    ...
    assert!(
        ...
        "cypher branch deleted: {branches:?}"
    );
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = cypher_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Cypher diff truncated"));
}
