...
//! created under `~/.cypher/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `cypher/<name>` branch. `CYPHER_WORKTREES_DIR` overrides the root.
//!
...
impl Repos {
    ...
    /// comes from `$CYPHER_WORKTREES_DIR` or `~/.cypher/worktrees`.
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
    let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
        std::io::ErrorKind::PermissionDenied => {
            EngineError::Other("Cypher doesn't have access to this folder on the device.".into())
        }
