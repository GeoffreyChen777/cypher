...
//!   the banner is attributed to Cypher with its icon. The API is deprecated
...
//!   center — those adopt the installed Cypher app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Cypher icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Cypher installed fall back to
...
//! - `CYPHER_DISABLE_NOTIFICATIONS` env kill-switch + the
...
pub fn post(title: &str, body: &str) {
    if cypher_env::var_os("DISABLE_NOTIFICATIONS").is_some() {
        return;
    }
...
mod delegate {
    ...
    pub(super) fn always_present() -> *mut Object {
        ...
        *DELEGATE.get_or_init(|| unsafe {
            let mut decl = ClassDecl::new("CypherNotifyDelegate", class!(NSObject))
                .expect("CypherNotifyDelegate registered twice");
            decl.add_method(
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Cypher app's id for
...
fn post_impl(title: &str, body: &str) {
    ...
    std::thread::spawn(move || {
        ...
        let result = std::process::Command::new("notify-send")
            .args(["--app-name=Cypher", "--", &title, &body])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status();
        match result {
