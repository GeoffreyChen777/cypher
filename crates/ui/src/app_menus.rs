...
/// ⌘Q / "Quit Cypher". `cx.quit()` runs the platform's standard quit routine,
...
/// The cypher menu bar. macOS renders this natively; mac-only entries are gated
...
pub fn app_menus() -> Vec<Menu> {
    ...
    if macos {
        app_items.extend([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide Cypher", Hide),
            MenuItem::action("Hide Others", HideOthers),
            MenuItem::action("Show All", ShowAll),
            MenuItem::separator(),
        ]);
    }
    app_items.push(MenuItem::action("Quit Cypher", Quit));

    let mut menus = vec![
        Menu::new("Cypher").items(app_items),
