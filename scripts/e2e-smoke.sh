...
# cypher-rpc e2e_driver example proving the doc-queued cross-device command path:
#
#   B queues a Run into the chat doc -> nudge -> A (host) executes via the mock
#   harness -> transcript + session status sync A -> edge -> B.
#
# Both engines run as the SAME user (alice@org1) on different devices — cypher's
# one-user-many-devices model; chat/device rooms are claim-on-first-join per user.
#
# Usage: scripts/e2e-smoke.sh
# Env:   CYPHER_E2E_EDGE_PORT (default 27640), CYPHER_E2E_KEEP_LOGS=1 to keep logs.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
command -v cargo >/dev/null 2>&1 || PATH="$HOME/.cargo/bin:$PATH"
EDGE_PORT="${CYPHER_E2E_EDGE_PORT:-27640}"
...
cleanup() {
  ...
  if [[ "${CYPHER_E2E_KEEP_LOGS:-0}" != "1" ]]; then
    rm -rf "$LOG_DIR"
  else
...
echo "build: cypher + e2e_driver"
# Two invocations: a single one with `--example` builds ONLY the example
# (the target filter applies across every -p), silently skipping the cypher
# bin — the smoke then dies on "No such file or directory".
(cd "$ROOT" && cargo build -q -p cypher)
(cd "$ROOT" && cargo build -q -p cypher-rpc --example e2e_driver)
CYPHER="$ROOT/target/debug/cypher"
