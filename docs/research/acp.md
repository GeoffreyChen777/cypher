...
- **Hermes + Pi registered** (2026-08-08): `AcpHarness::hermes()` runs Nous
  ...
  the static fallback. `AcpHarness::pi()` ran the pi coding
  agent (pi.dev) through the community `pi-acp` adapter (pinned 0.0.33,
  managed-install fallback; required the pi CLI itself,
  `@earendil-works/pi-coding-agent`; `PI_ACP_EXECUTABLE` overrode). Models
  rode pi's own provider config (catalog advertised a `default` pass-through
  entry); thinking ladder minimal→max mapped onto zeron's levels via the
  generic `thought_level` preference ladder ("off" has no zeron tier).
  **History (2026-08): pi moved off ACP onto its native RPC harness
  (`crates/harness/src/pi/`) — the pi-acp adapter path is deleted; see
  docs/research/pi-rpc.md.**
