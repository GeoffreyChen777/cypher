//! `cypher login` / `cypher logout` / `cypher status` — the standalone auth surface.
//!
//! Sign-in used to live only inside `cypher headless`, coupling authentication to
//! the long-running daemon. These commands work on the persisted session
//! (`{data_dir}/session.json`) and exit, so a service-managed `cypher headless`
...
use cypher_engine::{AuthState, Engine, EngineConfig, InstanceLock, WorkspaceScope};
    ...
    match scope {
        ...
        WorkspaceScope::Synced => match auth {
            ...
            AuthState::NeedsOrganization { user } => AccountStatus {
                mode: "synced",
                auth: format!(
                    "signed in as {} but no workspace selected — run `cypher login`",
                    user.email
                ),
                healthy: false,
            },
            AuthState::SignedOut => AccountStatus {
                mode: "synced",
                auth: "saved session is no longer valid — run `cypher login`".into(),
                healthy: false,
            },
        },
    }
...
/// `cypher login`: authenticate via the paste-code flow (and workspace
...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if let AuthState::SignedIn { user, org_id } = auth.state() {
        ...
        println!("Run `cypher logout` first to switch accounts.");
    ...
    match auth.state() {
        AuthState::SignedIn { user, org_id } => {
            ...
            println!(
                "Sync is ready. Start or restart Cypher to open the synced workspace; existing local sessions will stay local."
            );
        }
...
/// `cypher logout`: remove the persisted session.
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`cypher daemon stop`, or quit the Cypher app), or use the running UI instead."
        )
    })
}
...
mod tests {
    use super::*;
    use cypher_engine::{AuthUser, HarnessId};
