//! `cypher update` — check for and apply a newer release, natively (the same
...
pub async fn update(edge_url: &str, check_only: bool) -> anyhow::Result<()> {
    ...
    if !version_newer(&manifest.version, current) {
        println!(
            "cypher {current} is up to date (latest: {}).",
            manifest.version
        );
        return Ok(());
    }
    println!("cypher {current} → {} available", manifest.version);
    if check_only {
        std::process::exit(1);
    }

    match cypher_update::detect_install() {
        ...
        InstallKind::Unmanaged => {
            bail!(
                "this binary is not update-managed (source build or hand-copied).\n\
                 Linux: curl -fsSL https://edge.letscypher.app/install.sh | sh\n\
